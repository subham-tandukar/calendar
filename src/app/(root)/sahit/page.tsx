...
export async function generateMetadata(parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const title = data?.data?.sahit_meta_title || data?.data?.sahit_title_np
    const description = data?.data?.sahit_meta_description || data?.data?.sahit_description_np && removeHTMLTags(data?.data?.sahit_description_np)
    const ogTitle = `${title}`
    const imageUrl = data?.data?.sahit_og_image || data?.data?.site_og_image
    ...
    return {
        ...
        openGraph: {
            ...
            url: `${process.env.NEXT_PUBLIC_LIVE_URL}/sahit`,
            type: "article",
        },
        twitter: {
            title: ogTitle,
