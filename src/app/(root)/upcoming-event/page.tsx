...
export async function generateMetadata(parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const title = data?.data?.upcoming_event_meta_title || data?.data?.upcoming_events_title_np
    const description = data?.data?.upcoming_event_meta_description || data?.data?.upcoming_events_description_np && removeHTMLTags(data?.data?.upcoming_events_description_np)
    const ogTitle = `${title}`
    const imageUrl = data?.data?.upcoming_event_og_image || data?.data?.site_og_image
...
title: title ? title : "OK Calendar",
        ...
        openGraph: {
            ...
            url: `${process.env.NEXT_PUBLIC_LIVE_URL}/upcoming-event`,
            type: "article",
        },
        twitter: {
            title: ogTitle,
...
export default async function page() {
    ...
    return (
        <>
            <UpcomingEvent currentdate={data} />
        </>
    );
}