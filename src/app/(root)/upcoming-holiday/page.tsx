...
export async function generateMetadata() {
    ...
    const title = data?.data?.upcoming_holiday_meta_title || data?.data?.upcoming_holidays_title_np
    const description = data?.data?.upcoming_holiday_meta_description || data?.data?.upcoming_holidays_description_np && removeHTMLTags(data?.data?.upcoming_holidays_description_np)
    const ogTitle = `${title}`
    const imageUrl = data?.data?.upcoming_holiday_og_image || data?.data?.site_og_image
...
title: title ? title : "OK Calendar",
        ...
        openGraph: {
            ...
            url: `${process.env.NEXT_PUBLIC_LIVE_URL}/upcoming-holiday`,
            type: "article",
        },
        twitter: {
            title: ogTitle,
