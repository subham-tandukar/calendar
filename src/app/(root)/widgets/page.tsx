...
export async function generateMetadata(parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const title = data?.data?.widget_meta_title || data?.data?.widget_title_np
    const description = data?.data?.widget_meta_description || data?.data?.widget_description_np && removeHTMLTags(data?.data?.widget_description_np)
    const ogTitle = `${title}`
    const imageUrl = data?.data?.widget_og_image || data?.data?.site_og_image
    ...
    return {
        ...
        openGraph: {
            ...
            url: `${process.env.NEXT_PUBLIC_LIVE_URL}/widgets`,
            type: "article",
        },
        twitter: {
            title: ogTitle,
